...
male_mask = np.zeros(33)
female_mask = np.zeros(33)
for i in range(7,30):
    if i%2==0:
        male_mask[i] = 1.0
    else:
...
def train(train_loader, model, criterion, optimizer, epoch, target_rates):
    ...
    male_mask_t = torch.Tensor(male_mask)
    female_mask = torch.Tensor(female_mask)

    for i, (input, target, gender) in enumerate(train_loader):
        # input = input.view((input.size(0), input.size(2), input.size(3), input.size(1)))
        print(model.compute_average_flops_cost()/ 1e9 / 2)
        target = target.cuda(async=True)
        gender = gender.cuda(async=True)
        input = input.cuda()
